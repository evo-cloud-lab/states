...
var Program = Class(neuron.Program, {
    ...
    'neuron:watch': function (req, params) {
        if (typeof(params) == 'object') {
            for (var key in params) {
                params[key] ? this._watches.add(req.src, key, true) : this._watches.remove(req.src,key);
            }
        } else if (params && params.none == true) {
    ...
    'neuron:local.set': function (req, params) {
        if (typeof(params) == 'object') {
            this._states.localCommit(params, req.done);
        } else {
            req.fail(Errors.badParam('params'));
        }
    },

    'neuron:global.set': function (req, params) {
        if (typeof(params) == 'object') {
            this._states.globalCommit(params, req.done);
        } else {
            req.fail(Errors.badParam('params'));
        }
    },
